...
unresolved_branch_tail="$(
  sed -n 's/.* unresolved_branch_tail:\([0-9][0-9]*\) .*/\1/p' "${out}/star_tlb_accuracy.txt" | tail -n 1
)"
if [[ -z "${pdq_dispatch}" || -z "${pdq_commit}" || -z "${pdq_stall}" || -z "${pdq_high_watermark}" || -z "${seq_conflict}" || -z "${malformed_descriptor}" || -z "${unresolved_branch_tail}" ]]; then
  echo "ERROR: PDQ loss-free acceptance failed: missing simulator counters" >&2
  exit 2
fi
if (( pdq_dispatch != descriptor_count || pdq_commit != descriptor_count || pdq_stall != 0 || seq_conflict != 0 || malformed_descriptor != 0 )); then
  echo "ERROR: causal-input acceptance failed: descriptors=${descriptor_count} dispatch=${pdq_dispatch} commit=${pdq_commit} drops=${pdq_stall} seq_conflict=${seq_conflict} malformed_descriptor=${malformed_descriptor}" >&2
  exit 2
fi
...
echo "causal-input acceptance: PASS (dynamic PIM operands, malformed_descriptor=0; unresolved branch tail=${unresolved_branch_tail} is reported but excluded from resolved-branch accuracy)"
