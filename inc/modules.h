...
public:
  ...
  static B* create_instance(ModuleBuilder builder, C* parent)
  {
    ...
    try {
      B* instance_ptr = instance_map()[builder.get_name()]
                            .emplace_back(std::any_cast<std::function<std::unique_ptr<B>(ModuleBuilder builder)>>(module_map()[builder.get_model()])(builder))
                            .get();
...
struct replacement : public module_base<replacement, cache_module> {
   ...
   */
  virtual void update_replacement_state(uint32_t triggering_cpu, long set, long way, champsim::address full_addr, champsim::address ip,
   ...
   * \param type The access type.
   */
  virtual void replacement_cache_fill(uint32_t triggering_cpu, long set, long way, champsim::address full_addr, champsim::address ip,
