...
#include "module_decl.inc"
  ...
  struct prefetcher_module_concept {
    ...
    virtual uint32_t impl_prefetcher_cache_operate(champsim::address addr, champsim::address full_addr, champsim::address ip, bool cache_hit,
                                                   bool useful_prefetch, access_type type, uint64_t instr_id, uint32_t metadata_in) = 0;
  ...
  struct prefetcher_module_model final : prefetcher_module_concept {
    ...
    [[nodiscard]] uint32_t impl_prefetcher_cache_operate(champsim::address addr, champsim::address full_addr, champsim::address ip, bool cache_hit,
                                                         bool useful_prefetch, access_type type, uint64_t instr_id, uint32_t metadata_in) final;
  ...
  [[nodiscard]] uint32_t impl_prefetcher_cache_operate(champsim::address addr, champsim::address full_addr, champsim::address ip, bool cache_hit,
                                                       bool useful_prefetch, access_type type, uint64_t instr_id, uint32_t metadata_in) const;
  [[nodiscard]] uint32_t impl_prefetcher_cache_fill(champsim::address addr, long set, long way, bool prefetch, champsim::address evicted_addr,
...
template <typename... Ps>
uint32_t CACHE::prefetcher_module_model<Ps...>::impl_prefetcher_cache_operate(champsim::address addr, champsim::address full_addr,
                                                                              champsim::address ip, bool cache_hit, bool useful_prefetch,
                                                                              access_type type, uint64_t instr_id, uint32_t metadata_in)
{
  using return_type = uint32_t;
  [[maybe_unused]] auto process_one = [&](auto& p) {
    using namespace champsim::modules;
    /* Extended interface used by translation prefetch experiments. The full
     * byte address retains the page offset removed from addr; instr_id lets a
     * module suppress repeated tag-check callbacks. */
    if constexpr (prefetcher::has_cache_operate<decltype(p), champsim::address, champsim::address, champsim::address, bool, bool, access_type, uint64_t,
                                                uint32_t>)
      return return_type{p.prefetcher_cache_operate(addr, full_addr, ip, cache_hit, useful_prefetch, type, instr_id, metadata_in)};
